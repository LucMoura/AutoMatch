...
// =============================================
// 2. middleware/auth.ts — hybrid auth (tenta service key, fallback decode JWT)
// =============================================
...
export async function requireAuth(`,
  ...
  [
    ...
    `  // Strategy 1: try admin client (needs SUPABASE_SERVICE_KEY)
  try {
    const { data: { user }, error } = await supabaseAdmin.auth.getUser(token);
    if (!error && user) {
      req.userId = user.id;
...
// =============================================
// 3. routes/matches.ts — make recommendations public (no requireAuth)
// =============================================
